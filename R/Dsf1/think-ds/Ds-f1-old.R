...
##################################################################
#there are only 4 unusual values in norm.sum.crit.ds, we exclude them from comparisons
...
plot(seq(1:(max.iter-4)),norm.sum.crit.ds1 , type = "p",ylim=c(0,max(norm.sum.crit.ds1)),xlab="iter",ylab="Norm.Criterion Ds",pch=16,col="blue")
abline(h=max(norm.sum.crit.ds1),lty=2,col="red")
...
plot(seq(1:max.iter), sigmaEhat,ylim=c(0,0.6), type = "p",xlab="iter",ylab=expression(sigma[2]),pch=16,col="blue",cex.main=2, cex.lab=1.5, cex.axis=1.5)
